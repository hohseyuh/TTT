...
namespace game {
    ...
    class Rules {
    public:
        static Board initialState(int m = 3, int k = 3);
        ...
        static std::vector<Move> actions(const Board& b) noexcept;

        // Returns a new Board after applying action. Throws if illegal.
        static Board result(const Board& b, const Move& action);

        // Returns 'X' or 'O' if the winner exists, otherwise std::nullopt.
        static std::optional<char> winner(const Board& b) noexcept;
        ...
        static std::optional<int> utility(const Board& b) noexcept;
    };

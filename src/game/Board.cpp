...
Board::Board(int m_, int k_)
    : m(m_), k(k_), grid(static_cast<size_t>(m_ * m_), ' ') {
  // Validate sensible params
  if (m <= 0)
    throw invalid_argument("m must be > 0");
  if (k <= 0 || k > m)
    throw invalid_argument("k must be in 1..m");
}

char Board::at(int r, int c) const {
  if (!inBounds(r, c))
    throw out_of_range("Board::at out of bounds");
  return grid[static_cast<size_t>(index(r, c))];
}

void Board::set(int r, int c, char p) {
  if (!inBounds(r, c))
    throw out_of_range("Board::set out of bounds");
  if (p != 'X' && p != 'O' && p != ' ')
    throw invalid_argument("Invalid player char");
